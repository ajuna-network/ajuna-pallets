...
fn create_seasons<T: Config>(n: usize) -> Result<(), &'static str> {
	...
	frame_system::Pallet::<T>::set_block_number(
		Seasons::<T>::get(CurrentSeasonStatuses::<T>::get(1).season_id).unwrap().start,
	);
	Ok(())
}
...
fn create_avatars<T: Config>(name: &'static str, n: u32) -> Result<(), &'static str> {
	...
	let season_id = CurrentSeasonStatuses::<T>::get(1).season_id;
...
fn create_service_account_and_prepare_avatar<T: Config>(
	...
	let season = Seasons::<T>::get(CurrentSeasonStatuses::<T>::get(1).season_id).unwrap();
...
benchmarks! {
	...
	upgrade_storage {
		...
		let current_season_id = CurrentSeasonStatuses::<T>::get(1).season_id;
