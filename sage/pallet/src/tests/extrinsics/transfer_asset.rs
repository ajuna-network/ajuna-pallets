...
fn transfer_asset_rejects_transferring_to_self() {
	ExtBuilder::default().build().execute_with(|| {
		for who in [ALICE, BOB, CHARLIE, DAVE] {
			...
			assert_noop!(
				Sage::transfer_asset(RuntimeOrigin::signed(who), who, asset_id, NATIVE_PAYMENT),
...
fn transfer_asset_rejects_unowned_assets() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			Sage::transfer_asset(RuntimeOrigin::signed(ALICE), BOB, asset_id, NATIVE_PAYMENT),
...
fn transfer_asset_rejects_asset_not_matching_transfer_filters() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				Sage::transfer_asset(RuntimeOrigin::signed(BOB), ALICE, asset_id_1, NATIVE_PAYMENT),
